using System;
using System.Collections.Generic;
using Contracts;
using FinePrint.Contracts.Parameters;
using FinePrint.Utilities;
using Newtonsoft.Json.Linq;

namespace KSPArchipelago.Contracts.Primitives
{
    /// <summary>
    /// <c>{ "kind": "has_system", "system": "Generator", "label": "power generation" }</c>
    ///
    /// Maps to stock <see cref="VesselSystemsParameter"/> — the same check stock
    /// "build a base/station" contracts use. <c>system</c> is matched against the
    /// vessel's parts by KSP's contract-objective system (Vessel.HasValid-
    /// ContractObjectives): it must be a ContractObjectiveType backed by a module
    /// implementing IContractObjectiveModule (<c>"Generator"</c> = any solar panel
    /// / RTG / fuel cell; <c>"Antenna"</c> = any transmitter; plus Wheel,
    /// DockingPort, Grapple). A bare PartModule class name (e.g.
    /// <c>"ModuleScienceLab"</c>) never validates — such categories are matched by
    /// part name via has_any_part instead. One system per parameter; the generator
    /// emits several for an implicit AND. DLC/mod-robust and self-tracking.
    /// </summary>
    public sealed class HasSystemPrimitive : ContractPrimitiveBase<HasSystemPrimitive.Spec>
    {
        public override string Kind => "has_system";

        public struct Spec { public string System; public string Label; }

        protected override Spec Parse(JObject spec)
        {
            string system = (string)spec["system"];
            if (string.IsNullOrEmpty(system))
                throw new FormatException("has_system primitive missing 'system'");
            return new Spec { System = system, Label = (string)spec["label"] ?? system };
        }

        protected override ContractParameter BuildFrom(Spec p)
        {
            // ctor: (checkModuleTypes, checkModuleDescriptions, vesselDescription,
            //        mannedStatus, requireNew)
            return new VesselSystemsParameter(
                new List<string> { p.System },
                new List<string> { p.Label },
                p.Label,
                MannedStatus.ANY,
                false);
        }
    }
}
